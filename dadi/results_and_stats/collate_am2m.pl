...
my @files = grep { /AM2M\.txt$/ && -f "$in_dir/$_" } readdir($dh);
closedir $dh;

die "No *AM2M.txt files found in $in_dir\n" unless @files;
...
for my $fn (@files) {
    ...
    while (my $line = <$fh>) {
        ...
        $v{'Tam'}                   = $1 if $line =~ /^#Tam:\s*([^\s]+)\s*$/;
        $v{'Ts'}                    = $1 if $line =~ /^#Ts:\s*([^\s]+)\s*$/;
        $v{'p'}                     = $1 if $line =~ /^#p:\s*([^\s]+)\s*$/;
    }
    close $fh;

    # Compute Tam+Ts if numeric
    my ($tam, $ts) = ($v{'Tam'}, $v{'Ts'});
    if ($tam ne 'NA' && $ts ne 'NA' && $tam =~ $num_re && $ts =~ $num_re) {
        $v{'Tam+Ts'} = $tam + $ts;
    } else {
        $v{'Tam+Ts'} = 'NA';
    }

    # Capture Pop names for output filename
    $pop1_name //= $v{'Pop 1'} if $v{'Pop 1'} ne 'NA';
    $pop2_name //= $v{'Pop 2'} if $v{'Pop 2'} ne 'NA';

    # Enforce model
    if (defined $v{'Model'} && $v{'Model'} ne 'NA' && $v{'Model'} ne 'AM2M') {
        warn "Skipping $fn (Model is '$v{Model}', not 'AM2M')\n";
        next FILE;
    }
...
my $out_file = "$out_dir/$p1-$p2\_AM2M_summary.txt";
