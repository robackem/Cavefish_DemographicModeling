...
my @files = grep { /IM2M\.txt$/ && -f "$in_dir/$_" } readdir($dh);
closedir $dh;

die "No *IM2M.txt files found in $in_dir\n" unless @files;
...
for my $fn (@files) {
    ...
    # Enforce model
    if (defined $v{'Model'} && $v{'Model'} ne 'NA' && $v{'Model'} ne 'IM2M') {
        warn "Skipping $fn (Model is '$v{Model}', not 'IM2M')\n";
        next FILE;
    }
...
my $out_file = "$out_dir/$p1-$p2\_IM2M_summary.txt";
